...
class SpotifyChart
  ...
  def get_url(region)
    ...
    url = "#{BASE_URL}/#{region}/weekly/latest"
    return url
  end
  ...
  def get_first_track_info(music_hash)
    ...
    return_string = "#{track_hash["track_name"]} by #{track_hash["artist_name"]} from the album #{track_hash["album_name"]}"
  end
